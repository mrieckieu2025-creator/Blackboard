...
class BlackboardClient:
    def __init__(self, bb_url: str, username: str = "", password: str = "", cookie_string: str = ""):
    ...
    def login(self) -> bool:
        """Try cookie-based auth first, then API login, then browser."""
    ...
    def _try_cookie_login(self) -> bool:
        """Use session cookies captured from the user's browser."""
        try:
            ...
            for pair in self.cookie_string.split(";"):
                ...
                if "=" in pair:
                    ...
                    self.session.cookies.set(name.strip(), value.strip(), domain="blackboard.ie.edu")
            # Verify the session works
            resp = self.session.get(
                f"{self.bb_url}/learn/api/public/v1/users/me",
                timeout=10,
            ...
            if resp.status_code == 200:
                self._api_available = True
                return True
            # Even if the API check fails, cookies may still work for scraping
